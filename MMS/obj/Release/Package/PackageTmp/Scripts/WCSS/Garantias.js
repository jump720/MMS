...
$.MMS.Garantias = function (mod, items, garantia, dataItems, dataMotivo, files, dataCausa) {
    mod = mod.toLowerCase();
    ...
    if (garantia != null) {
        ...
        $.get("/api/Devoluciones/GetCliente/" + garantia.ClienteId)
            .done(function (result) {
                if (result) {
                    ...
                    $("#Garantia_ClienteId").select2(select2Options);
                }
            })
    ...
    select2MotivoOptions = {
        ...
        ajax: {
            type: "GET",
            url: function (params) {
                return "/api/MotivosPQRS/BuscarMotivoPQRS?q=" + encodeURIComponent(params.term) + "&t=2"
            },
            delay: 300,
            processResults: function (data, params) {
                params.page = params.page || 1;
    ...
    if (mod == "create" || mod == "edit") {
        var estado = $("#Garantia_Estado").val();
