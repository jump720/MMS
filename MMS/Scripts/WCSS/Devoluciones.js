...
var templateMotivoPQRS;
var select2MotivoPQRSOptions;
$.MMS.Devoluciones = function (mod, items, devolucion, dataItems, dataMotivo, files, dataCausa) {
    mod = mod.toLowerCase();
    ...
    if (devolucion != null) {
        ...
        $.get("/api/Devoluciones/GetCliente/" + devolucion.ClienteId)
            .done(function (result) {
                if (result) {
                    ...
                    $("#Devolucion_ClienteId").select2(select2Options);
                }
            })
    ...
    templateItems = function (data) {
        //console.log(data.UnidadEmpaque);
    ...
    //SELECT2 PARA Motivo PQRS
    templateMotivoPQRS = function (data) {
    ...
    select2MotivoPQRSOptions = {
        ...
        ajax: {
            type: "GET",
            url: function (params) {
                return "/api/MotivosPQRS/BuscarMotivoPQRS?q=" + encodeURIComponent(params.term) + "&t=1"
            },
            delay: 300,
            processResults: function (data, params) {
                params.page = params.page || 1;
        ...
        templateResult: function (repo) {
            ...
            return templateMotivoPQRS(repo.data);
        },
        templateSelection: function (repo) {
            if (!repo.id)
                return repo.text;
    ...
    //$("#_selectMotivoPQRSId").select2(select2MotivoPQRSOptions);
    ...
    $("#_selectMotivoPQRSId").on('select2:select', function (e) {
        ...
        DataSelectTipo = $(this).select2('data')[0].data;

    });

    //SELECT2 PARA TIPO DEVOLUCION
    ...
    $("#btnSave").click(function (e) {
        if (ValidaFormItems()) {
            if (idx == null) {
                ...
                item.MotivoPQRSId = DataSelectTipo;
...
function setDataModal(item) {
    if (idx != null) {
        ...
        $.get("/api/Plantillas/GetItem/" + item.ItemId)
            .done(function (result) {
                if (result) {
                    ...
                    console.log(_items[0].data.UnidadEmpaque);
        ...
        $.get("/api/MotivosPQRS/GetMotivoPQRS?id=" + item.MotivoPQRSId.Id + "&t=1")
            .done(function (result) {
                if (result) {
                    ...
                    //select2MotivoPQRSOptions.data = motivos;
                    $("#_selectMotivoPQRSId").val(result.Id).change();
                    DataSelectTipo = result;
        ...
        $.get("/api/CausaPQRS/GetCausaPQRS?id=" + item.CausaPQRSId.Id + "&t=1")
           .done(function (result) {
               if (result) {
                   var causa = [];
                   causa.push({
...
function ValidaFormItems() {
    ...
    if (DataSelectTipo == null) {
        msgError("Campo Motivo requerido");
        result = false;
    } else if (DataSelectTipo.Id == "") {
        msgError("Campo Motivo requerido");
        result = false;
    }
