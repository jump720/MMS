...
{
    public class TipoIndustriaController : ApiBaseController
    {
        ...
        public async Task<IHttpActionResult> Index(FormDataCollection form)
        {
            try
            {
                ...
                int count = await db.TipoIndustrias
                    .Where(m => m.Nombre.Contains(search) || m.Id.ToString().Contains(search))
                    .CountAsync();

                var data = await db.TipoIndustrias
                    .Where(m => m.Nombre.Contains(search) || m.Id.ToString().Contains(search))
        ...
        [HttpPost]
        [ApiAuthorizeAction]
        public async Task<IHttpActionResult> Create(TipoIndustria model)
        {
            try
            {
                db.TipoIndustrias.Add(model);
        ...
        [HttpPost]
        [ApiAuthorizeAction]
        public async Task<IHttpActionResult> Edit(TipoIndustria model)
        {
            try
            {
